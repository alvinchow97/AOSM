...
def updateItemStockQuantity(itemId, stockQuantity):
    ...
    items = openItemFile()
    # TODO assume itemId is 1, replace the 2 later with the quantity later, validation last do (last week)
    for item in items:
        if (item[0] == str(1)):
...
def updateItemStockQuantityByReduce(itemId, reduceNo):
    items = openItemFile()
    # TODO replace the 1 with the itemId later, replace the 2 later with the quantity to be reduce later, validation last do (last week)
    for item in items:
        if (item[0] == str(1)):
